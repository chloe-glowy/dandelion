...
// @ts-ignore the lib does not have TS declarations yet
import {
  ...
  Position,
  Suggestion,
} from 'src/shared/presenter_utils/mentions/types';
...
const parseValue = (
  ...
  if (partTypes.length === 0) {
    ...
    let i = 0;
    for (const result_ of matches) {
      const result = result_ as RegExpMatchArray;
      ...
      if (resultIndex + result[0].length !== value.length) {
